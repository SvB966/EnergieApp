...
FREQS = {
    '5T':  {'label': 'Elke 5 minuten', 'minutes': 5,    'seconds': 300,      'pandas': '5T'},
    '15T': {'label': 'Elke 15 minuten', 'minutes': 15,   'seconds': 900,      'pandas': '15T'},
    'H':   {'label': 'Per uur',         'minutes': 60,   'seconds': 3600,     'pandas': 'H'},
    'D':   {'label': 'Dagelijks',       'minutes': 1440, 'seconds': 86400,    'pandas': 'D'},
    'W':   {'label': 'Wekelijks',       'minutes': 10080,'seconds': 604800,   'pandas': 'W'},
    'ME':  {'label': 'Maandelijks',     'minutes': 43200,'seconds': 2592000,  'pandas': 'M'},
    'Y':   {'label': 'Jaarlijks',       'minutes': 525600,'seconds': 31536000,'pandas': 'A'},
...
def detect_auto_frequency(datetime_index):
    ...
    for key, freq in FREQS.items():
        if abs(freq['seconds'] - mode_diff) < 10:
            return key
    return 'H'
...
def resample_dataframe(df, freq_key, method='sum'):
    ...
    if pandas_freq is None:
        return df
    # Sum by default, or pass 'mean' or 'first' etc. via method argument
